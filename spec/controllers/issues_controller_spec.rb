...
describe IssuesController do
  ...
  context 'when using REST API for getting issue tags' do
    ...
    it 'show action should not include tags when not explicitly specified' do
      get :show, params: { id: issue_1.id, format: :json }
    ...
    it 'show action should include tags when requested' do
      get :show, params: { id: issue_2.id, include: 'tags', format: :json }
      project_response = response.parsed_body['issue']
      expect(response).to have_http_status(:ok)
    ...
    it 'index should not include tags when not explicitly specified' do
      ...
      api_response = response.parsed_body['issues']
      expect(response).to have_http_status(:ok)
    ...
    it 'index should include own tags for each issue' do
      tags_1 = ActsAsTaggableOn::Tag.where(name: %w[a1 a2]).collect { |tag| { 'id' => tag.id, 'name' => tag.name } }
      tags_2 = ActsAsTaggableOn::Tag.where(name: %w[b3 b4]).collect { |tag| { 'id' => tag.id, 'name' => tag.name } }
      ...
      expect(api_issue_1).not_to be_nil
      expect(api_issue_2).not_to be_nil
